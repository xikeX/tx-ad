...
import functools
from faiss_demo_gpu import run_faiss_ann_search
...
class BaseDataset():
    ...
    def _init_feat_info_2(self, feat_statistics, feature_types):
        ...
        """
        self.USER_SPARSE_FEAT = {k: feat_statistics[k] for k in feature_types['user_sparse']}
        self.USER_CONTINUAL_FEAT = feature_types['user_continual']
        self.ITEM_SPARSE_FEAT = {k: feat_statistics[k] for k in feature_types['item_sparse']}
        self.ITEM_CONTINUAL_FEAT = feature_types['item_continual']
        self.USER_ARRAY_FEAT = {k: feat_statistics[k] for k in feature_types['user_array']}
        self.ITEM_ARRAY_FEAT = {k: feat_statistics[k] for k in feature_types['item_array']}
        EMB_SHAPE_DICT = {"81": 32, "82": 1024, "83": 3584, "84": 4096, "85": 3584, "86": 3584}
        self.ITEM_EMB_FEAT = {k: EMB_SHAPE_DICT[k] for k in feature_types['item_emb']}  # 记录的是不同多模态特征的维度
    ...
    def fill_missing_feat(self, feat, item_id, is_user=False):
        ...
        if is_user:
            ...
            return {"sparse_feature": sparse_feature, "continual_feature": continual_feature, "array_feature": array_feature}
        else:
            # user_array_feature_range
            sparse_feature.append(item_id)
            ...
            # return sparse_feature, continual_feature, array_feature, mm_emb
            return {"sparse_feature": sparse_feature, "continual_feature": continual_feature, "array_feature": array_feature, "mm_emb": mm_emb}
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, base_dataset, sample_index=[], neg_sample_size=30, max_padding_size=100):
        super().__init__()
        ...
        self.hard_sample = np.random.randint(1, self.base_dataset.itemnum, size=2+ (self.base_dataset.itemnum + 1) * neg_sample_size, dtype=np.uint16)
    ...
    def create_faiss(self, dataset_vector, dataset_id, query_vector, orginal_id, faiss_metric_type=1):
        ...
        # ================== 3. 创建并初始化 hnswlib 索引 ==================
        indices, avg_score = run_faiss_ann_search(
            dataset_vector_file_path=None,
            dataset_id_file_path=None,
            query_vector_file_path=None,
            result_id_file_path=None,
            dataset_vector=dataset_vector,
            dataset_id=dataset_id,
            query_vector=query_vector,
            query_ann_top_k=query_ann_top_k,
            faiss_M=faiss_M,
        )
        ...
        for cnt, a_id in enumerate(orginal_id, start=1):
            a_id = int(a_id)
            for retrive_id in indices:
                for b_id in retrive_id[5:]:
                    b_id = int(b_id)
                    if a_id == b_id:
                        continue
                    self.hard_sample[a_id * self.neg_sample_size + self.tp[a_id]] = b_id
                    self.tp[a_id] = (self.tp[a_id] + 1) % self.neg_sample_size

                    self.hard_sample[b_id * self.neg_sample_size + self.tp[b_id]] = a_id
                    self.tp[b_id] = (self.tp[b_id] + 1) % self.neg_sample_size
            # 计算时间
            if cnt % 1000 == 0:
                end_time = round(time.time())
                use_time = format_time(end_time - start_time)
    ...
    def _random_neq(self, item_id):
        # 从难样本中随机取
        pos = random.randint(0, self.neg_sample_size)
        return self.hard_sample[item_id * self.neg_sample_size + pos]
    ...
    def build_example(self, user_sequence):
        ...
        seq_feat_sparse = np.full([self.max_padding_size + 1, len(self.base_dataset.item_feature_default_value["sparse_feature"])], self.base_dataset.item_feature_default_value["sparse_feature"],
                                  dtype=np.int32)
        ...
        seq_feat_array = np.full([self.max_padding_size + 1, len(self.base_dataset.item_feature_default_value["array_feature"])], self.base_dataset.item_feature_default_value["array_feature"],
                                 dtype=np.int32)
        seq_feat_mm_embs = np.full([self.max_padding_size + 1, len(self.base_dataset.item_feature_default_value["mm_emb"])], self.base_dataset.item_feature_default_value["mm_emb"], dtype=np.float32)

        pos_feat_sparse = np.full([self.max_padding_size + 1, len(self.base_dataset.item_feature_default_value["sparse_feature"])], self.base_dataset.item_feature_default_value["sparse_feature"],
                                  dtype=np.int32)
        pos_feat_continual = np.full([self.max_padding_size + 1, len(self.base_dataset.item_feature_default_value["continual_feature"])],
                                     self.base_dataset.item_feature_default_value["continual_feature"], dtype=np.float32)
        pos_feat_array = np.full([self.max_padding_size + 1, len(self.base_dataset.item_feature_default_value["array_feature"])], self.base_dataset.item_feature_default_value["array_feature"],
                                 dtype=np.int32)
        pos_feat_mm_embs = np.full([self.max_padding_size + 1, len(self.base_dataset.item_feature_default_value["mm_emb"])], self.base_dataset.item_feature_default_value["mm_emb"], dtype=np.float32)

        neg_feat_sparse = np.full(
            [(self.max_padding_size + 1) * self.neg_sample_size, len(self.base_dataset.item_feature_default_value["sparse_feature"])], self.base_dataset.item_feature_default_value["sparse_feature"],
            dtype=np.int32)
        neg_feat_continual = np.full(
            [(self.max_padding_size + 1) * self.neg_sample_size, len(self.base_dataset.item_feature_default_value["continual_feature"])],
            self.base_dataset.item_feature_default_value["continual_feature"], dtype=np.int32)
        neg_feat_array = np.full(
            [(self.max_padding_size + 1) * self.neg_sample_size, len(self.base_dataset.item_feature_default_value["array_feature"])], self.base_dataset.item_feature_default_value["array_feature"],
            dtype=np.int32)
...
class ValidDataset(torch.utils.data.Dataset):
    ...
    def build_example(self, user_sequence):
        ...
        if user_sparse_feature is None:
            user_sparse_feature, user_continual_feature, user_array_feature = self.base_dataset.user_feature_default_value['sparse_feature'], self.base_dataset.user_feature_default_value[
                'continual_feature'], self.base_dataset.user_feature_default_value['array_feature']
    ...
    def collate_fn(batch):
        ...
        return_batch['label'] = np.array(label)
        return return_batch
