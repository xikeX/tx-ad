...
# 本地自定义模块
from baseline_model_v3_infonce_ctcvr import BaselineModel as DownstreamModel
...
def get_args():
    ...
    parser.add_argument('--train_name', type=str, default="v3_ctcvr", help='训练名称')
...
def evalution(model, valid_loader, global_step, args, writer):
    ...
    with torch.no_grad():
        for batch in valid_loader:
            token_type, next_token_type, next_action_type, seq_feat, pos_feat, pos_1_feat, neg_feat, user_feat = \
                batch['token_type'], batch['next_token_type'], batch['next_action_type'], batch['seq_feat'], batch['pos_feat'], batch['pos_1_feat'], batch['neg_feat'], batch['user_feat']
            # 移动到设备
            ...
            for m in [seq_feat, pos_feat, pos_1_feat, neg_feat, user_feat]:
                for k in m:
                    m[k] = m[k].to(args.device)
            # 前向传播
            output = model(token_type, next_token_type, next_action_type, seq_feat, pos_feat, pos_1_feat, neg_feat, user_feat)
