...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def build_example(self, user_sequence):
        ...
        pos_feat_mm_embs = np.full([self.max_padding_size + 1,len(self.base_dataset.item_feature_default_value["mm_emb"])],
                                  self.base_dataset.item_feature_default_value["mm_emb"], dtype=np.float32)
        # 点击样本
        pos_1_feat_sparse = np.full([self.max_padding_size + 1,len(self.base_dataset.item_feature_default_value["sparse_feature"])],
                                  self.base_dataset.item_feature_default_value["sparse_feature"], dtype=np.int32)
        pos_1_feat_continual = np.full([self.max_padding_size + 1,len(self.base_dataset.item_feature_default_value["continual_feature"])]
                                      ,self.base_dataset.item_feature_default_value["continual_feature"], dtype=np.float32)
        pos_1_feat_array = np.full([self.max_padding_size + 1,len(self.base_dataset.item_feature_default_value["array_feature"])],
                                  self.base_dataset.item_feature_default_value["array_feature"], dtype=np.int32)
        pos_1_feat_mm_embs = np.full([self.max_padding_size + 1,len(self.base_dataset.item_feature_default_value["mm_emb"])],
                                  self.base_dataset.item_feature_default_value["mm_emb"], dtype=np.float32)
        ...
        last_click_feat = None
        if next_act_type==1:
            last_click_feat = next_feat
        ...
        for record_tuple in reversed(ext_user_sequence[:-1]):
            ...
            if next_token_type[idx] ==1 and last_click_feat is not None:
                next_token_type[idx]=3
            if next_token_type[idx]==3:
                pos_1_feat_sparse[idx], pos_1_feat_continual[idx], pos_1_feat_array[idx], pos_1_feat_mm_embs[idx] = \
                    last_click_feat['sparse_feature'], last_click_feat['continual_feature'], last_click_feat['array_feature'], last_click_feat['mm_emb']
        ...
        return token_type, next_token_type, next_action_type, seq_feat_sparse, seq_feat_continual, seq_feat_array, seq_feat_mm_embs, \
                ...
                pos_1_feat_sparse, pos_1_feat_continual, pos_1_feat_array, pos_1_feat_mm_embs,\
    ...
    def collate_fn(batch):
        ...
        return_batch['pos_1_feat'] = {
            "item_sparse_feature": torch.tensor(np.stack(pos_1_feat_sparse,axis=0)),
            "item_continual_feature": torch.tensor(np.stack(pos_1_feat_continual,axis=0)),
            "item_array_feature": torch.tensor(np.stack(pos_1_feat_array,axis=0)),
            "item_mm_embs": torch.tensor(np.stack(pos_1_feat_mm_embs,axis=0)),
        }
